// any application handling by browerser developed using html is called web page 
/*
frames - Helping  us in integrating one html page to anither html page. 
By using tag - iframe
...
test.only("Handling frame ", async function({page}){

    await page.goto("https://demo.automationtesting.in/Frames.html")
    // frameLocato() - which helps us in entering inside the iframe to select the locator

    const  framePage = page.frameLocator("#Single iframe")

    await framePage.locator("div.col-xs-6 input").first().fill("Testing")

    await expect (framePage.locator("div.col-xs-6 input")).toHaveValue("Testing")
